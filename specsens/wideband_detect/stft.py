...
class Stft:
    ...
    def stft(self, sig, f_sample, normalized=True, dB=True):
        ...
        assert len(sig) == self.n, 'Signal segment and fft bin dont match.'
        f, s = self.fft(self.window(sig), f_sample)
        ps = self.psd(s, f_sample, normalized)
        if dB:
            ps = 10. * np.log10(ps)  # power signal dB
        return f, ps
    ...
    def spectogram(self, sig, f_sample, normalized=True, dB=True):
        ...
        tl = []
        xl = []
        segs = filter(lambda x: len(x) == 1024,
              [sig[i:i + 1024] for i in range(0, len(sig), 1024)])
