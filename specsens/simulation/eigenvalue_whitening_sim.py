...
from specsens import filter


def whitening_samples(f_sample, length_sec, itrs, noise_power, noise_uncert,
                      cov_size, f_min, f_max, seeds):
    ...
    # list for correlated noise matrices
    ...
    for _ in range(itrs):
        ...
        noi_fil = filter.fir_bandpass(noi,
                                      f_min=f_min,
                                      f_max=f_max,
                                      f_sample=f_sample)
        # get correlation matrix
        w_mat = eigen_detector.whitening_mat(noi_fil, l=cov_size)

        # store matrix in list
        w_mat = np.reshape(w_mat, (1, cov_size, cov_size))
        w_mats = np.append(w_mats, w_mat, axis=0)

    # average over correlation matrix list
    w_mat = np.mean(w_mats, axis=0)

    # list for detection differences
    diffs = np.array([])
    ...
    for _ in range(itrs):
        ...
        # apply eigendetecor to noise only
        eng_noi = eigen_detector.mme(x=noi, l=cov_size)

        # apply eigendetecor to filtered noise
        eng_noi_fil = eigen_detector.mme(x=noi_fil, l=cov_size, w_mat=w_mat)

        # store differences
        diffs = np.append(diffs, eng_noi_fil - eng_noi)

    # average over difference list
    bias = np.mean(diffs, axis=0)

    return w_mat, bias


def train_whitening(gens, itrs, f_sample, noise_power, length_sec,
                    noise_uncert, seed, num_procs, cov_size, f_min, f_max):
    ...
    f = partial(whitening_samples, f_sample, length_sec, itrs, noise_power,
                noise_uncert, cov_size, f_min, f_max)
    ...
    # 'unwrap' res tuples
    w_mats = [r[0] for r in res]
    biases = [r[1] for r in res]

    print('biases mean', np.mean(biases, axis=0))
    print('biases std ', np.std(biases, axis=0))

    return np.mean(w_mats, axis=0), np.mean(biases, axis=0)


def generation(f_sample, length_sec, itrs, noise_power, signal_power,
               noise_uncert, threshold, num_bands, f_center, band_to_detect,
               cov_size, f_min, f_max, w_mat, bias, seeds):
    ...
    for _ in range(itrs):
        ...
        both_fil = filter.fir_bandpass(both,
                                       f_min=f_min,
                                       f_max=f_max,
                                       f_sample=f_sample)

        # maximum-minimum eigenvalue detector
        eng = eigen_detector.mme(x=both_fil, l=cov_size, w_mat=w_mat)
        eng -= bias
...
def eigenvalue_whitening_sim(
    ...
    print('---- Training prewhitening matrix ----')
    print('Using %d processes on %d cores' % (num_procs, mp.cpu_count()))

    w_mat, bias = train_whitening(gens=pre_gens,
                                  itrs=pre_itrs,
                                  f_sample=f_sample,
                                  noise_power=noise_power,
                                  length_sec=length_sec,
                                  noise_uncert=noise_uncert,
                                  seed=seed,
                                  num_procs=num_procs,
                                  cov_size=cov_size,
                                  f_min=f_min,
    ...
    f = partial(generation, f_sample, length_sec, itrs, noise_power,
                ...
                band_to_detect, cov_size, f_min, f_max, w_mat, bias)
