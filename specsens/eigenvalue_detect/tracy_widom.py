# This is not my work, all credits go to Yao-Yuan Mao from Stanford University. (see https://gist.github.com/yymao/7282002)
...
def _dlnf_dx(x, N, a, b, c, u=0, v=0):
    s = x**1.5
    return -(3. * a * s * s + 1.5 * b * s + c + 1.5 * (s * u + v + v) /
             ((s + u) * s + v)) / x
...
def _finv(y, N, a, b, c, u=0, v=0):
    ...
    xa[~flag] = np.inf
    for i in range(3):
        ca = np.log(xa[flag]) * c + logyn - np.log1p(u / xa[flag]**1.5 +
                                                     v / xa[flag]**3)
        xa[flag] = ((np.sqrt(b * b - 4. * a * ca) - b) /
                    (a * 2.) if a else ca / (-b))**(2. / 3.)
    return xa
