...
class WirelessMicrophone:
    def __init__(self, f_sample, num_samples=None, t_sec=None, seed=None):
        assert np.all(f_sample > 0.), 'f_sample must be greater than 0'
    ...
    def signal(self, f_center, f_deviation, f_modulation, power=1., dB=True):
        assert np.all(f_deviation > 0.), 'f_deviation must be greater than 0'
        assert np.all(f_modulation > 0.), 'f_modulation must be greater than 0'
